const lightProbeImageRegions = {
	...
	sponza: {
		center: { x0: 0.42, x1: 0.58, y0: 0.38, y1: 0.62 },
		leftCorridor: { x0: 0.12, x1: 0.32, y0: 0.36, y1: 0.72 },
		rightCorridor: { x0: 0.68, x1: 0.88, y0: 0.36, y1: 0.72 },
		lowerCenter: { x0: 0.34, x1: 0.66, y0: 0.62, y1: 0.88 }
	}
};
...
export function captureLightProbeImageRegions( image, regionSet = 'cornell' ) {

	const regions = {};
	const selectedRegions = lightProbeImageRegions[ regionSet ] ?? lightProbeImageRegions.cornell;

	for ( const [ name, region ] of Object.entries( selectedRegions ) ) {
...
export function createLightProbeImageArtifactPressure( regions ) {
	...
	const maxDarkPixelRatio = Math.max( ...regionValues.map( region => region.darkPixelRatio ) );
	const maxBlackPixelRatio = Math.max( ...regionValues.map( region => region.blackPixelRatio ) );
	const maxCellEdgeContrast = Math.max( ...regionValues.map( region => region.cellEdgeContrast ) );
	const luminanceFloor = Math.min( ...regionValues.map( region => region.luminance.p01 ) );

	return {
		darkTailRatio: roundLightProbeMetric( maxDarkPixelRatio ),
		blackTailRatio: roundLightProbeMetric( maxBlackPixelRatio ),
