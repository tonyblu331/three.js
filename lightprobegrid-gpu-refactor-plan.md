...
- Phase 108 closes the sixteenstudio control diff: its useful pieces are the compact GPU SH atlas product shape, dedicated runtime node integration, Sponza-scale demo pressure, and atlas padding between packed SH sub-volumes. Those pieces do not address the remaining residual gate because the open failure is rendered masked receiver energy after compatibility shaping, not SH sub-volume atlas bleed.
- Phase 109 defines the next valid receiver-boundary sources: authored geometry/material GPU node data and setup-authored region/layer ownership metadata. Proof pixels, CPU readback, scalar/WebGL comparison, brightness validity, Cornell divider coordinates, and old adjacent-fallback payloads remain rejected.
- Phase 110 selects the metadata layout for the next candidate: keep probe ownership in existing `probeMeta.b`, feed receiver boundary class through GPU node/value inputs, preserve default behavior when absent, and defer new GPU buffers/textures until a many-class source proves the need.
- Phase 111 remains held until a generic Phase 109 source is wired into the Phase 110 layout and can apply classification before interpolation. No prototype should scale irradiance down to hide leak; it must move residual input facts while preserving same-side and correct-bounce guards.
- Phase 111 audit rejects a shallow receiver-boundary wrapper prototype. The runtime already accepts `receiverBoundaryLayerMask` and `receiverBoundaryWeight`, and the harness already measured the whole-receiver `receiverBoundaryWeight = 1` path; it supports the current guards but leaves residual at `0.9377`. A new prototype must add represented receiver/material/geometry state, not rename the existing descriptor.
- Phase 112 keeps promotion held: no new candidate reduces the zero-oracle residual below `0.9377`, so the bounded proof package remains `OPEN` with 15 supported / 1 open gate.
